# Task
# In the join() example, create a join condition where the combined length of the first 
# and last name is less than or equal to the job's age limit.

from pyspark.sql import SparkSession
from pyspark.sql.functions import collect_list, length, concat
...
# Adding condition: total length of name should be less than or equal to age limit
job_eligibility = df1.crossJoin(df2).filter(
    (df1.age >= df2.age_limit) &
    (length(df1.name) <= df2.age_limit)
)
